...
Важно про заголовки шагов (steps[].title):
- title — что делает родитель в этом шаге, коротко и по делу.
- title и phrase должны совпадать по смыслу: не «перепрыгивай» через начало фразы.
- Если фраза начинается с наблюдения факта или чувства родителя («Я заметил…», «Мне неприятно…», «Я вижу…»), отрази это в title, а не только цель «узнать мнение ребёнка».
- Хорошие примеры: «Назвать факт без обвинения», «Сказать, что заметили, и спросить версию», «Описать проблему и услышать ребёнка».
- Плохие примеры: «Выяснить, как ребёнок оценивает ситуацию» — когда по сути шаг начинается с вашего наблюдения/чувства; «Обсудить последствия» — когда сначала ещё только обозначаете границу.
- Не делай title абстрактным процессом («коммуникация», «диалог», «проработка») — пиши конкретное действие родителя.`;
...
В steps[].title пиши действие родителя так, чтобы оно совпадало с phrase (если phrase начинается с факта/чувства — title тоже про это).`;
}
