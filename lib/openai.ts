...
- closing — «Фраза для завершения»: ТОЛЬКО на ПОСЛЕДНЕМ шаге. Одна спокойная фраза, чтобы закрыть разговор по цели (благодарность + что дальше / когда вернуться / без обвинений). Не дублируй основную phrase шага. На остальных шагах — null;
...
1) understand — сначала узнать, что произошло и почему. Не переходи к правилам, последствиям и решениям, пока позиция ребёнка не прояснена.
   ...
   closing на финале: поблагодарить за честный рассказ и оставить дверь открытой (без «накажем потом»).
   Если в ситуации есть враньё, сокрытие или расхождение версий (сказал одно — было другое):
   НЕ перескакивай сразу к «почему так поступил» / страхам / поддержке.
   Нужна явная последовательность:
   а) открытие без допроса;
   б) спокойно назвать расхождение фактов / дать правде выйти (родитель сообщает, что знает, или мягко сверяет версии) — отдельный шаг;
   в) только после этого — мотивы, чувства, поддержка.
   В шаге «б» нельзя формулировать фразы так, будто правда уже признана ребёнком.
...
2) agree — услышать → вместе найти вариант → правило + пробный период.
   ...
   closing на финале: спасибо + срок проверки («через неделю посмотрим») + «если не сработает — пересмотрим без обвинений».
...
3) boundary — ясно сообщить позицию. Несогласие и эмоции можно. Обсуждается НЕ сама граница, а детали реализации (discuss только про детали).
   ...
   closing на финале: граница остаётся + можно позже уточнить детали соблюдения, без торга об самой границе.
...
4) announce — спокойно сообщить уже принятое решение, объяснить, помочь с реакцией. Совместный поиск решения не обязателен.
   ...
   closing на финале: спасибо что выслушал + решение принято + можно задать вопросы позже.
...
5) support — быть рядом, выслушать, признать переживания, спросить какую помощь нужно.
   ...
   closing на финале: спасибо + «я рядом» + без давления возвращаться.

6) trust — факт → спокойно обозначить расхождение / правду → причины → влияние на доверие → как быть честнее дальше.
   Не склеивай «версию» и «причину» в два одинаковых «почему». Не пропускай момент, когда правда названа вслух.
   closing на финале: спасибо за честный разговор + мягкая проверка через несколько дней без упрёков.

7) other — одна конкретная цель родителя; не склеивай типы.
   closing на финале под конкретную цель родителя.

Дисциплина диалога:
- каждый шаг — отдельная функция; следующий шаг не предполагает ответы, которых ещё не было;
- если родитель знает правду, а ребёнок ещё нет / врёт — не строй следующий шаг так, будто расхождение уже признано;
- не прыгай от нейтрального открытия сразу к мотивам вранья или страхам наказания: сначала шаг, где правда аккуратно выходит наружу;
...
export function planUserPrompt(input: PlanRequest) {
  ...
  const sit = `${input.situation} ${input.goalText}`.toLowerCase();
  const involvesLie =
    /(врал|врёт|соврал|скрыл|обманул|не сказал правд|не рассказал правд|расхожд|сказал.{0,40}(друг|школ|дом|парк)|вместо)/i.test(
      sit,
    ) || /(правд|обман|лож)/i.test(sit);
...
${involvesLie
    ? "В ситуации есть враньё/сокрытие: обязателен отдельный шаг, где правда спокойно выходит наружу (назвать расхождение фактов), ДО шагов про мотивы, страхи и поддержку. Не предполагай, что ребёнок уже признался."
    : ""}
...
export function normalizePlan(
  ...
  const steps = (raw.steps || [])
    ...
    .map((s) => {
      ...
      return repairStepDialogue({
        ...
        closing: (s as PlanStep).closing
          ? String((s as PlanStep).closing).trim()
          : undefined,
  ...
  const deduped = dedupeInviteSteps(steps, ctx?.goalKind);
  const withClosing = deduped.map((step, i, arr) => {
    if (i !== arr.length - 1) {
      return { ...step, closing: undefined };
    }
    return {
      ...step,
      closing:
        step.closing?.trim() ||
        (ctx?.goalKind ? closingPhraseForGoal(ctx.goalKind) : undefined),
    };
  });
