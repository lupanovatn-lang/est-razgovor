...
- из описания ситуации, НЕ из категории «Тема»;
- коротко и по-человечески: «Ночи за компьютером», «Скрытая двойка»;
...
export function finalizePlanTitle(
  ...
  const looksLikeStep =
    /^(начните|скажите|спросите|выслушайте|предложите|договоритесь|объясните|обратите|признайте|уточните|поговорите|поддержите|спокойно|чётко|коротко)\b/i.test(
      next,
    );
  if (!next || isCategory || next.length < 3 || looksLikeStep) {
